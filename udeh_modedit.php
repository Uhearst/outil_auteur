...
$url->param('isglobal', $isglobal);


if ($subquestionid && $explorationid) {
    $url->param('subquestionid', $subquestionid);
    $url->param('explorationid', $explorationid);
}

if ($evaluationid) {
    $url->param('evaluationid', $evaluationid);
}
...
if (!empty($add)) {
    ...
    if($explorationid) {
        ...
        $data->explorationid = $explorationid;
    } else {
    ...
    navigation_node::override_active_url(new moodle_url('/course/format/udehauthoring/udeh_modedit.php', array('update'=>$update, 'return'=>1)));
...
if ($mform->is_cancelled()) {
    if ($subquestionid) {
        redirect(new moodle_url('/course/format/udehauthoring/redact/subquestion.php', ['id' => $subquestionid]));
    } else if($isglobal === 1) {
        redirect(new moodle_url('/course/format/udehauthoring/redact/globalevaluation.php', ['course_id' => $course->id]));
    } else {
        redirect(new moodle_url('/course/format/udehauthoring/redact/evaluation.php', ['id' => $evaluationid]));
    }
    ...
    if ($subquestionid) {
        ...
        if(!$exptool) {
            $exptool = new \format_udehauthoring\model\explorationtool_plan();
            $exptool->audehexplorationid = $explorationid;
            $exptool->courseid = $course->id;
        }
        $exptool->tooltype = $module->name;
        if(property_exists($fromform, 'id')) {
            $exptool->toolid = $fromform->id;
        } else {
            $exptool->toolid = $fromform->instance;
        }
        $exptool->save();
    } else {
        $evaltool =  \format_udehauthoring\model\evaluationtool_plan::instance_by_audehevaluationid($evaluationid);
        if(!$evaltool) {
            $evaltool = new \format_udehauthoring\model\evaluationtool_plan();
            $evaltool->audehevaluationid = $evaluationid;
            $evaltool->courseid = $course->id;
        }
        $evaltool->tooltype = $module->name;
        if(property_exists($fromform, 'id')) {
            $evaltool->toolid = $fromform->id;
        } else {
            $evaltool->toolid = $fromform->instance;
        }
        $evaltool->save();
    }
    ...
    } else if (array_key_exists('submitbutton3', $_POST)) {
        if ($subquestionid) {
            redirect(new moodle_url('/course/format/udehauthoring/redact/subquestion.php', ['id' => $subquestionid]),
                get_string('toolsaved', 'format_udehauthoring'), null,
    ...
    $PAGE->requires->js_call_amd('format_udehauthoring/toolHelper', 'initEditTools',
        array(array($subquestionid, $explorationid, $evaluationid, $isglobal)));
