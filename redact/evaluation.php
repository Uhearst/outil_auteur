...
$evaluationid = optional_param('id', 0, PARAM_INT); // This is required.
$evaluationplan = \format_udehauthoring\model\evaluation_plan::instance_by_id($evaluationid);
$courseplan =  \format_udehauthoring\model\course_plan::instance_by_id($evaluationplan->audehcourseid);
...
$form = new \format_udehauthoring\form\redact_evaluation(
    ...
    array('coursetitle' => $courseplan->title,
        ...
        'teachingobectives' =>
            \format_udehauthoring\model\teachingobjective_plan::instance_all_by_course_plan_id($courseplan->id),
...
if($evaluationplan->toolcmid !== null) {
    ...
    if ($info->cms[$evaluationplan->toolcmid]) {
        $PAGE->requires->js_call_amd(
            'format_udehauthoring/evaluationHelper',
            'initPhpEvaluationValidation',
            array($info->cms[$evaluationplan->toolcmid]->name)
        );
    } else {
