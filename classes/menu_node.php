...
class menu_node
{
    ...
    public static $TYPE_EXPTOOL = 7;
    public static $TYPE_EVALTOOL = 8;
    ...
    private function __construct($url, $parent, $courseid, $moduleindex = null, $subquestionindex = null, $isevaluations = false, $toolindex = null) {
    ...
    static function build_tree(\course_modinfo $course_modinfo, target $target) {
        ...
        // Evaluations tools
        $tools_idnumbers = $target->filter_cmidnumbers(array_keys($cms), $course_modinfo->get_course_id(), '\d+', false, true, '\d+');
        foreach ($tools_idnumbers as $idnumber) {
            $cm = $cms[$idnumber];

            $data = $target->unpack_cmidnumber($idnumber);
            $parent = $root->children[count($root->children) - 1]->children[$data->moduleindex];
        ...
        $tools_idnumbers = $target->filter_cmidnumbers(array_keys($cms), $course_modinfo->get_course_id(), '\d+', '\d+', false, '\d+');
        foreach ($tools_idnumbers as $idnumber) {
            $cm = $cms[$idnumber];

            $data = $target->unpack_cmidnumber($idnumber);

            $parent = $root->children[$data->moduleindex]->children[$data->subquestionindex];
     ...
     */
    public function find($searchdata) {
        if ($this->courseid === $searchdata->courseid &&
            $this->moduleindex === $searchdata->moduleindex &&
            $this->subquestionindex === $searchdata->subquestionindex &&
            $this->isevaluations === $searchdata->isevaluations &&
            $this->toolindex === $searchdata->toolindex) {
            return $this;
        }

        foreach ($this->children as $child) {
            $result = $child->find($searchdata);
            if (!is_null($result)) {
                return $result;
            }
        }
    ...
    public function type() {
        ...
        } else if (!is_null($this->toolindex)) {
            return $this->isevaluations ?
                self::$TYPE_EVALTOOL :
                self::$TYPE_EXPTOOL;
